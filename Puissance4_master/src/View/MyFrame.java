...
import Controller.CouleurJoueur1;
import Controller.CouleurJoueur2;
import Controller.MultiListener;
import Controller.NewListener;
import Controller.Save;
import Controller.SetAliListener;
import Controller.SetColonneListener;
import Controller.SetLigneListener;
...
public class MyFrame  extends Frame{
	...
	public MyFrame (Controller control) {
		this.setTitle("Le puissance 4 du pirate");			//On gere les parametres standards de la fenetre
		this.setSize(LARGEUR, HAUTEUR);
		this.setLayout(new BorderLayout());
		MyCanvas mc = new MyCanvas(control);
		
		Button nouveau = new Button("Nouvelle partie");		//On creer les differents boutton 
		Button undo = new Button ("Undo");	
		Panel p = new Panel();
		
		MenuBar menu = new MenuBar();					//Gestion des menus
		...
		MenuItem s2 = new MenuItem("Recuperer");
		...
		Menu ia = new Menu ("Solo/Multi");
		MenuItem solo = new MenuItem ("Solo");
		MenuItem multi = new MenuItem ("Multijoueur");
		
		c1.addActionListener(new SetColonneListener(control,3));			//On rajoute les differentes fonctions aux menus et boutons 
		...
		solo.addActionListener(new MultiListener (control, true));
		multi.addActionListener(new MultiListener ( control, false));
		...
		this.setMenuBar(menu);							//On rajoute a la fenetre les differents elements
