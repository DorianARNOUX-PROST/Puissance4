...
public class IA {
	...
	// Simule un coup du joueur passé en paramètre sur la grille
	public static void play(Grid grille, int joueur) {
		...
		// Vérification victoire en un coup
		...
		if (!quit) {
			...
			// Anticipation sur plusieurs coups jusqu'à 4
			else {
				for (int i = 0; i < test.getNbColonne() && !quit; i++) {
					test = new Grid(grille);
					test.play(i, joueur);
		...
		// Au cas où si rien a été trouvé (Fin de partie par exemple)
		if (!quit || !played) {
			boolean quit2 = false;
			while(!quit2) {
				if (grille.play(ThreadLocalRandom.current().nextInt(0, grille.getNbColonne()), joueur)) {
